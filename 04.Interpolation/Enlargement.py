# HW6-(i) (Implement Image Enlargement & filling use NN interpolation)
...
# Step3 : Filling(NN interpolation)
NN_mask = [[1, 1, 3],
           [1, 1, 0],
           [0, 0, 0]]
...
for j in range(len(I2[1])):
    for i in range(len(I2[0])):
        tmp = 0
        for b in range(len(NN_mask)):
            for a in range(len(NN_mask)):
                if (j+b-1 >= new_column) or (i+a-1 >= new_row):
                    tmp += 0 * NN_mask[b][a]
                else:
                    tmp += I2[j+b-1][i+a-1] * NN_mask[b][a]
        # reDraw image
        enlarged_image.putpixel((i, j), int(tmp))
